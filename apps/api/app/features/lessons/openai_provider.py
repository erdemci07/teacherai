...
class OpenAILessonProvider:
    ...
    async def generate_lesson_plan(self,analysis:VisionAnalysis,correction_feedback:str|None=None,teaching_context=None)->LessonProviderResult:
        if not self.api_key: raise LessonProviderConfigurationError
        payload={"analysis":analysis.model_dump(exclude={"debug"}),"verification_feedback":correction_feedback,"teaching_context":teaching_context.model_dump(mode="json") if teaching_context else None}
        try:
