...
async def generate(body:GenerateLessonRequest,request:Request,service:Annotated[LessonService,Depends(get_lesson_service)],students:Annotated[StudentService,Depends(get_student_service)],user:Annotated[AuthenticatedUser|None,Depends(optional_user)]):
    context=students.context(user,body.analysis.topic) if user else None
    return ApiResponse(data=await service.generate(body.analysis,context,getattr(request.state,"request_id",body.analysis.request_id)))
