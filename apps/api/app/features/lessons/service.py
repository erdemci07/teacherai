...
class LessonService:
    ...
    async def generate(self,analysis:VisionAnalysis)->GeneratedLesson:
        started=perf_counter(); correction=False
        plan=await self._plan(analysis,None)
        ...
        if verification.contradiction:
            ...
            plan=await self._plan(analysis,feedback); verification=self.mathai.verify(plan)
    ...
    async def _plan(self,analysis,feedback):
        started=perf_counter(); result=await self.provider.generate_lesson_plan(analysis,feedback)
