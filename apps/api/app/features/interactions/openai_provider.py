...
class OpenAIInteractionProvider:
    ...
    async def adapt(self,action:Action,lesson:LessonPlan,hint_level:int,teaching_context=None)->AdaptiveDraft:
        ...
        payload={"action":action,"hint_level":hint_level,"lesson":lesson.model_dump(by_alias=True,exclude={"source_analysis":{"debug"}}),"teaching_context":teaching_context.model_dump(mode="json") if teaching_context else None}
