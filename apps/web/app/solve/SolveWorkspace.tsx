...
export function SolveWorkspace() {
  ...
  return <div className="solvePage" onPaste={pasted}>
    ...
    <div className="solveGrid">
      ...
      <aside className="solveOutput" aria-label="TeacherAI anlatımı">{busy && <AnalysisLoading uploading={state === 'uploading'} stage={state} />}{!busy && !result && !error && <div className="resultEmpty"><div className="emptyBoardIcon" aria-hidden="true"><span>∑</span></div><h2>Çözümün burada görünecek</h2><p>TeacherAI sorunu çözdüğünde kullanılan kuralı, çözüm adımlarını ve dikkat etmen gereken noktaları burada anlatacak.</p></div>}{result && <><TeacherBoard result={result} /><InteractionPanel lesson={result.lesson} /><ShareSolution result={result} /><button className="textToggle" onClick={() => setShowText(!showText)} aria-expanded={showText}>{showText ? 'Metin anlatımını gizle' : 'Metin olarak göster'}</button>{showText && <LessonText lesson={result.lesson} />}<SolutionFeedback result={result} />{process.env.NEXT_PUBLIC_TEACHERAI_DEBUG === 'true' && <details className="technicalDetails"><summary>Teknik detaylar</summary><pre>{JSON.stringify(result, null, 2)}</pre></details>}</>}</aside>
    </div>
  </div>;
}
